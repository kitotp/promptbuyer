...
export default function TaskDetails() {
    /** URL-параметр /tasks/[taskId] */
    const { taskId } = useParams<{ taskId: string }>();

    /** достаём массив задач, загруженных списком */
    const queryClient = useQueryClient();
    const tasks = queryClient.getQueryData<Task[]>(['tasks']);

    /** ищем нужную задачу в кеше */
    const task = useMemo(
        () => tasks?.find((t) => t.id === taskId) ?? null,
        [tasks, taskId],
    );

    /** локальное состояние для файла и превью */
    ...
    return (
        <div className="mx-auto max-w-xl space-y-6 px-4 py-6">
            ...
            <section className="space-y-4">
                <h2 className="text-lg font-medium">Загрузите скриншот ПОЛНОГО экрана где будет видно дату и время выполнения а также оставленный промпт и ответ на него.</h2>
                ...
                <button
                    disabled={!file}
                    className="w-full rounded-xl bg-emerald-600 px-4 py-3 text-white shadow
                     disabled:cursor-not-allowed disabled:bg-gray-400"
                    onClick={() => alert('TODO: upload')}
                >
                    Отправить
                </button>
            </section>
